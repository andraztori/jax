...
run_local_wheel_smoke_test() {
  if [[ "$JAXCI_BUILD_JAXLIB" != "false" ]]; then
    return 0
  fi

  local expected_wheel_versions_json
  expected_wheel_versions_json="$(python3 ci/parse_wheel_metadata.py --wheel-dir=dist)"

  echo "Running downloaded wheel smoke test..."
  bazel "${common_bazel_test_args[@]}" \
    "--test_env=JAXCI_EXPECTED_WHEEL_VERSIONS_JSON=$expected_wheel_versions_json" \
    --test_output=all \
    --local_test_jobs=1 \
    //tests:local_wheel_smoke_test_gpu
}
