...
class MpmdMapTest(PallasSCTest):
  ...
  def test_parallel_subkernels_semaphores(
      self, use_tc_tiling, full_core_spec, signalling_direction
  ):
    ...
    if not full_core_spec and signalling_direction in ("both", "scs_to_tec"):
      # No compiler support at the moment.
      errtype, msg = ValueError, r"Body failed to verify.*"
    elif full_core_spec and signalling_direction in ("both", "scs_to_tec"):
      # TODO(rdyro): Add support for explicitly specifying the core axis when
      # signalling a TEC.
      errtype = NotImplementedError
      msg = r"Addressing a VectorSubcore on a non-same.*"
    elif signalling_direction in ("tec_to_scs",):
      # No compiler support at the moment.
      errtype = jax.errors.JaxRuntimeError
      msg = r".*Failed to verify lowered MLO module.*"
    else:
      raise ValueError("Unexpected combination of flags")

    with self.assertRaisesRegex(errtype, msg):
