...
class PallasCallTCGen05Test(PallasTCGen05Test):
  ...
  def test_collective_scaled_matmul_m128(self):
    m = n = 128
    ...
    scratch_shapes = [
        plgpu.SMEM((m_block, k), in_jax_dtype, transforms=transforms),
        plgpu.SMEM((n_block, k), in_jax_dtype, transforms=transforms),
        plgpu.SMEM(((m_block + 127) // 128, k // (scale_block * 4), 32, 16), scale_jax_dtype),
        plgpu.SMEM((1, k // (scale_block * 4), 64, 16), scale_jax_dtype),
        plgpu.Barrier(num_arrivals=4),
        plgpu.Barrier(orders_tensor_core=True),
        plgpu.TMEM((m_block, n), out_jax_dtype, collective=True),
        plgpu.TMEM((128, k // scale_block), scale_jax_dtype,
                   layout=plgpu.TMEMLayout.SCALES_LAYOUT, collective=True),
        plgpu.TMEM((256, k // scale_block), scale_jax_dtype,
                   layout=plgpu.TMEMLayout.SCALES_M64_COLLECTIVE_LAYOUT, collective=True),
    ]
